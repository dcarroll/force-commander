...
sforce.Xml.prototype.writeValue = function (sobjectNs, writer, name, val) {
    if (val === null) {
        if (this._xsiType !== 'metadata') {
            //console.log("sforce.xml: " + JSON.stringify(this, null, 2));
    ...
    if (val.toXml) {
        ...
        for (var i=0; i<val.length; i++) {
            this.writeValue(sobjectNs, writer, name, val[i]);
        }
    } else {
...
//        }
    ...
    this.findOutput = function() {
        if (this.output === null) {
            this.output = console; //this.win.document.getElementById("output");
        }
        return this.output;
    };
        ...
        for (var i = 0; i < sforce.internal._connections.length; i++) {
            ...
            if (holder !== null) {
                ...
                if (holder.timedout) {
                    ...
                    //console.log("\n" + JSON.stringify(holder.connection, null, 4));
                    ...
                    if (success) { //(holder.connection.responseXML && holder.connection.responseXML.documentElement) {
                        var parseString = require('xml2js').parseString;
                        parseString(holder.connection.responseText, function(err, result) {
                            holder.callback.httpCallback(result, success);
                        });
//                        holder.callback.httpCallback(holder.connection.responseXML.documentElement, success);
                        ...
                        parseString(holder.connection.responseText, function(err, result) {
                            if (typeof err !== 'null') {
                                if (typeof result["soapenv:Envelope"]["soapenv:Body"][0].loginResponse !== 'undefined')
                                    holder.callback.httpCallback(result, true);
                                else 
                                    holder.callback.httpCallback(result);
...
sforce.XmlReader.prototype.getEnvelope = function() {
    if (this.envelope) {
        ...
        return this.envelope["soapenv:Envelope"];
    }
...
sforce.SoapTransport.prototype.send = function(url, writer, isArray, connectionCallback) {
    ...
    if (this.isAsync) {
        ...
        transport.send(writer.toString(), this, true, connectionCallback.timeout);
    } else {
        transport.send(writer.toString(), this, false);
        if (this.fault !== null) {
            return this["fault"];
        }
        return this.result;
...
sforce.SoapTransport.prototype.xml2jsObject = function(responseText) {
    var parseString = require('xml2js').parseString;
    parseString(holder.connection.responseText, function(err, result) {
...
sforce.SoapTransport.prototype.httpCallback = function(response, success) {
    try {
        if (success === true) {
            var envelope = response["soapenv:Envelope"];
            var body = envelope["soapenv:Body"];
            var operation;
            var operationName;
            for (var key in body[0]) {
                operation = body[0][key];
                operationName = key;
            }
            ...
            var resultArray =  operation[0].result;
            ...
            var envelope = response["soapenv:Envelope"];
            var body = envelope["soapenv:Body"];
            this.fault = body[0]["soapenv:Fault"];
            this.sendFault(this.fault);
        }
    } catch(fault) {
...
sforce.Connection = function() {
    ...
    this.serverUrl = "/services/Soap/u/27.0";
};
...
//sforce.Connection.prototype.sforceNs = "http://soap.sforce.com/2006/04/metadata";
//sforce.Connection.prototype.sobjectNs = "http://soap.sforce.com/2006/04/metadata";

sforce.Connection.prototype.writeOne = function (writer, name, value, sobjectNs) {
    if (value === null) {
        writer.writeNameValueNode(name, null);
    } else if (value.toXml) {
        //console.log("Writing value toXml: " + JSON.stringify(value, null, 2));
...
sforce.Connection.prototype.createObject = function (sobjects, callback) {
    var arg = new sforce.internal.Parameter("metadata", sobjects, true);
    return this.invoke("create", [arg], true, callback);
};

sforce.Connection.prototype.deleteObject = function (sobjects, callback) {
    var arg = new sforce.internal.Parameter("metadata", sobjects, true);
    return this.invoke("delete", [arg], true, callback);
};

sforce.Connection.prototype.describeMetadata = function (apiVersion, callback) {
    var arg = new sforce.internal.Parameter("apiVersion", apiVersion, true);
    return this.invoke("describeMetadata", [arg], true, callback);
};

sforce.Connection.prototype.retrieveMetadata = function (retrieveRequest, callback) {
    var arg = new sforce.internal.Parameter("retrieveRequest", retrieveRequest, true);
    return this.invoke("retrieve", [arg], true, callback);
};

sforce.Connection.prototype.deployMetadata = function (deployRequest, callback) {
    var arg = new sforce.internal.Parameter("ZipFile", deployRequest.ZipFile, false);
    var arg2 = new sforce.internal.Parameter("DeployOptions", deployRequest.DeployOptions, false);
    return this.invoke("deploy", [arg, arg2], true, callback);
};

sforce.Connection.prototype.checkRetrieveStatus = function (Id, callback) {
    var arg = new sforce.internal.Parameter("Id", Id, true);
    return this.invoke("checkRetrieveStatus", [arg], true, callback);
};

sforce.Connection.prototype.checkStatus = function (Id, callback) {
    var arg = new sforce.internal.Parameter("Id", Id, true);
    return this.invoke("checkStatus", [arg], true, callback);
};

sforce.Connection.prototype.checkDeployStatus = function (Id, callback) {
    var arg = new sforce.internal.Parameter("Id", Id, true);
    return this.invoke("checkDeployStatus", [arg], true, callback);
};

sforce.Connection.prototype.listMetadata = function (queries, asOfVersion, callback) {
    var arg1 = new sforce.internal.Parameter('queries', queries, true);
    var arg2 = new sforce.internal.Parameter("asOfVersion", asOfVersion, true);
    return this.invoke("listMetadata", [arg1, arg2], true, callback);
};
...
sforce.Connection.prototype.getUserInfo = function(callback) {
    var result = this.invoke("getUserInfo", [], false, callback);
    //console.log("CONNRESULT " + result);
    return result;
};
...
sforce.Connection.prototype.writeHeader = function(writer, headerNs) {
    ...
    if (this.loginScopeHeader !== null) {
        //console.log("LS Header: " + JSON.stringify(this.loginScopeHeader, null, 2));
...
sforce.Connection.prototype._invoke = function(method, args, isArray, callback, namespaces, url, headerNs, sobjectNs) {
    ...
    for (var i = 0; i < args.length; i++) {
        ...
        if (arg.value === null) {
            ...
            //console.log("Arg " + arg.name + " is an array with a length of: " + arg.value.length);
            for (var j = 0; j < arg.value.length; j++) {
                var obj = arg.value[j];
                //console.log("The arg value is: " + JSON.stringify(obj, null, 2))
                if (!obj) {
            ...
            //console.log("We are writing what we think is just a value");
            if (typeof arg.value === 'object') {
                // Need to recursively write this
                writer.writeStartElement(arg.name);
                for (var key in arg.value) {
                    //console.log("Writing key: " + key);
                    this.writeOne(writer, key, arg.value[key], sobjectNs);
                }
                writer.writeEndElement(arg.name);   
            } else {
    ...
    if (sforce.debug.trace) {
        ...
        sforce.debug.log(writer.toString());
...
sforce.connection.serverUrl = "https://login.salesforce.com/services/Soap/u/27.0";

if (typeof(__sfdcSessionId) != "undefined") {
    sforce.connection.sessionId = __sfdcSessionId;
}
exports.sforce = sforce;
exports.sforce.connection = sforce.connection;
